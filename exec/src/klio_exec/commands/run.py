...
class KlioPipeline(object):
    ...
    def _verify_setup_manifest(self):
        # verify that MANIFEST.in includes klio-job-run-effective.yaml and
        # setup.py references it
        ...
        include_line = f"include {config_core.RUN_EFFECTIVE_CONFIG_FILE}"
        if os.path.exists(manifest_file_path):
            with open(manifest_file_path, "r") as r:
                data = r.read()
                if config_core.RUN_EFFECTIVE_CONFIG_FILE not in data:
                    logging.warning(
                        "runtime config not detected in "
                        f"MANIFEST.in. Please ensure that "
                        f"the line '{include_line}'"
                        " is in your job's MANIFEST.in"
                    )
        else:
            logging.warning(
                "No MANIFEST.in detected.  Please ensure that"
                f" this file exists and includes the line '{include_line}'"
            )
